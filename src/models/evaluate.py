...
def walk_forward_evaluation(df: pd.DataFrame) -> pd.DataFrame:
    """Walk-forward con ventana expansiva: entrena con todos los años
    anteriores al año de test, para cada año de test."""
    results = []
    ...
    for test_year in test_years:
        ...
        logger.info(
            f"Test {test_year} -> Accuracy: {acc:.3f}, F1-macro: {f1:.3f}, LogLoss: {ll:.3f}"
        )
        results.append(
            {
                "TestYear": test_year,
...
def plot_metrics(results_df: pd.DataFrame) -> Path:
    ...
    sns.lineplot(
        data=results_df, x="TestYear", y="Accuracy", marker="o", label="Accuracy", ax=ax
    )
    sns.lineplot(
        data=results_df, x="TestYear", y="F1Macro", marker="s", label="F1 Macro", ax=ax
    )
