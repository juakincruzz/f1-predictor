...
years = list(
    range(config.get("data.start_year", 2010), config.get("data.end_year", 2026) + 1)
)
...
with tab_pred:
    ...
    if not model_exists:
        st.error(
            "No se encontró modelo entrenado. Ejecuta `make train` para generar `models/xgb_model.json`."
        )
    else:
        df_year = build_features_for_year(selected_year)
        if df_year.empty:
            st.warning(
                "No hay datos procesados para esta temporada. Ejecuta 'make data' y 'make features'."
            )
        else:
            preds = predict_for_race(
                df_year, selected_year, selected_round, model_path=str(model_path)
            )
            if preds.empty:
                ...
                for i, row in top3.iterrows():
                    st.write(
                        f"**{row['Abbreviation']}** ({row['TeamName']}) — {row['PredictedLabel']} — Podio: {row['PodiumProbability']:.1%}"
                    )
...
with tab_live:
    ...
    st.markdown(
        "Monitoreo de sesiones en curso (requiere FastF1 live donde esté disponible)."
    )
